...
function parseAccountFields(formData: FormData) {
  ...
  return { name, type, balance, currency, color }
}
...
export async function createAccount(formData: FormData) {
  ...
  await db.account.create({ data: { userId: session.userId, ...data } })
...
export async function updateAccount(id: string, formData: FormData) {
  ...
  const data = parseAccountFields(formData)
  await db.account.updateMany({ where: { id, userId: session.userId }, data })
  revalidatePath('/capital')
}
